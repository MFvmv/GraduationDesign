...
export function removeUidAndStatus(obj) {
...
export function convertToNestedObject(path, value) {
  const cutStr = path.split('.');
  const param = {};
  let cursor = param;

  for (let i = 1; i < cutStr.length; i++) {
      if (i === cutStr.length - 1) {
          let tmp = { data: null };
          tmp.data = JSON.parse(JSON.stringify(value));
          const handleObj = removeUidAndStatus(tmp);
          cursor[cutStr[i]] = JSON.stringify(handleObj);
          break;
      }
      cursor[cutStr[i]] = {};
      cursor = cursor[cutStr[i]];
  }
