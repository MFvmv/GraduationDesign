...
import api from "@/api/BE/api/DrugconsumptionApi.js"
...
import { formAddplus_callback,formUpdate_callback } from './Drugconsumption.js';
...
export default {
 ...
 setup() {
  ...
  const formAddplus = convertApiToItems({
   ...
   },formAddplus_callback({schemaContext,fapi,mateStr:"drugconsumption"}));
   ...
   const tableAttr = parseTableConfig({
      ...
      consumptionID: {name:"药品消耗ID",width:100}, // 药品消耗唯一标识
      drugID: {name:"药品ID",width:80}, // 药品唯一标识（外键）
      patientID: {name:"病人ID",width:80}, // 病人唯一标识（外键）
      quantityUsed: {width:80}, // 使用数量
      lastUpdated: null, // 更新时间
      count: {width:80}, // 用时总量
   })

   const tablePetName = "药品记录"
   return {
     api,
     tableAttr,
     formAdd,
